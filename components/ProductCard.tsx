...
export function ProductCard({
  ...
  return (
    <article data-motion-card className="group soho-interactive-card soho-lift flex min-w-0 flex-col overflow-hidden rounded-2xl border bg-white shadow-sm transition duration-300 hover:border-[#049ca5]/30 focus-within:border-[#049ca5]/50 focus-within:shadow-xl sm:rounded-3xl">
      <div className="aspect-[4/3] bg-neutral-100">
        {product.image_url ? (
          <img src={product.image_url} alt={product.name} loading="lazy" className="h-full w-full object-cover transition duration-700 ease-out group-hover:scale-[1.07]" />
        ) : (
      ...
      <div className="flex min-w-0 flex-1 flex-col p-2.5 sm:p-4">
        ...
        <button onClick={onAdd} className="mt-auto rounded-xl pt-3 outline-none focus-visible:ring-4 focus-visible:ring-cyan-200 sm:rounded-2xl">
          <span className="soho-shimmer flex min-h-9 w-full items-center justify-center rounded-xl bg-[#049ca5] px-2 text-[10px] font-black leading-tight text-white transition duration-200 hover:-translate-y-0.5 hover:bg-[#037f86] hover:shadow-lg active:scale-[0.98] sm:min-h-11 sm:rounded-2xl sm:px-3 sm:text-sm">
