...
import { LayoutDashboard, ClipboardList, Mail, CalendarDays, History, Package, Clock3 } from 'lucide-react';
...
import { DateRange, aggregateOrders, filterOrdersByRange, presetRange } from '@/lib/reporting';
...
function DashboardBars({ title, data, range, onRange, valueKey, money = false, color }: any) {
  ...
  return <div className="rounded-[28px] border border-black/10 bg-white p-6 shadow-sm">
    <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-2xl font-black">{title}</h2><RangeSelector compact value={range} onChange={onRange} /></div>
...
export default function AdminPage() {
  ...
  async function login(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err?.message || 'No se pudo conectar con Supabase. Revisa las variables .env.local y que el proyecto esté activo.');
    }
  }
  ...
  function playNewOrderSound() {
    try {
      ...
      console.warn('No se pudo reproducir sonido de nuevo pedido:', error);
    }
  }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, (payload: any) => {
        if (payload.eventType === 'INSERT' && payload.new?.id && !knownOrderIdsRef.current.has(payload.new.id)) {
          knownOrderIdsRef.current.add(payload.new.id);
          if (!firstRealtimeLoadRef.current) playNewOrderSound();
        }
  ...
  const daySummary = useMemo(() => {
    const validOrders = dayOrders.filter((order) => order.status !== 'cancelled');
  ...
  const historyDays = useMemo(() => {
    ...
    return Array.from(grouped.entries()).sort((a,b) => b[0].localeCompare(a[0])).map(([day,list]) => {
      const valid = list.filter((o) => o.status !== 'cancelled'); const total = revenue(list); const top = productStats(list)[0];
          ...
          {section === 'dashboard' && (
            <section>
              <div className="mb-7 flex flex-wrap items-start justify-between gap-4">
                <div><h1 className="text-4xl font-black tracking-tight text-neutral-950">Dashboard</h1><p className="mt-2 text-sm font-semibold text-neutral-700">Ventas y actividad real de la web. Los periodos largos se agrupan automáticamente para mostrarse completos, sin desplazamiento horizontal.</p></div>
            ...
            <section>
              ...
              <div className="rounded-[28px] border border-black/10 bg-white p-6 shadow-sm">
                ...
                <div className="mt-6 rounded-[28px] border border-black/10 bg-cyan-50 p-6">
                  ...
                  <p className="mt-2 text-sm font-semibold text-neutral-700">Estos valores se usan en el informe PDF para gestoría. Gestión, hosting y dominio se muestran a 0 durante los tres primeros meses desde la fecha de inicio.</p>
            ...
            <section>
              ...
              <div className="mt-5 grid gap-4">
                {filteredAdminProducts.map((product) => (
                  <div key={product.id} className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm">
                    <div className="grid gap-5 lg:grid-cols-[112px_minmax(0,1fr)] 2xl:grid-cols-[112px_minmax(0,1.2fr)_minmax(300px,0.9fr)_180px] xl:items-start">
                      <div className="flex justify-center xl:justify-start">{product.image_url ? <img src={product.image_url} alt={product.name} className="h-28 w-28 rounded-3xl object-cover" /> : <div className="grid h-28 w-28 place-items-center rounded-3xl bg-cyan-50 text-xs font-black text-neutral-500">SOHO</div>}</div>
                      <div className="grid min-w-0 gap-3">
                        <label className="grid gap-1 text-xs font-black text-neutral-600">Nombre<input value={product.name || ''} onChange={(e) => patchProduct(product.id, { name: e.target.value })} className="w-full rounded-2xl border border-black/10 bg-neutral-50 p-3 text-sm font-bold" placeholder="Nombre" /></label>
                        <label className="grid gap-1 text-xs font-black text-neutral-600">Descripción<textarea value={product.description || ''} onChange={(e) => patchProduct(product.id, { description: e.target.value })} className="min-h-24 w-full resize-y rounded-2xl border border-black/10 bg-neutral-50 p-3 text-sm font-semibold" placeholder="Descripción" /></label>
                      </div>
                      <div className="grid min-w-0 gap-3 sm:grid-cols-2 2xl:grid-cols-1">
                        ...
                        <label className="grid min-w-0 gap-1 text-xs font-black text-neutral-600">Enlace de imagen<input value={product.image_url || ''} onChange={(e) => patchProduct(product.id, { image_url: e.target.value })} className="w-full min-w-0 rounded-2xl border border-black/10 bg-neutral-50 p-3 text-sm font-semibold" placeholder="https://ejemplo.com/imagen.jpg" /></label>
                        <label className="cursor-pointer rounded-2xl border border-dashed border-black/15 bg-white px-3 py-3 text-center text-xs font-black text-slate-700 transition hover:bg-cyan-50"><input type="file" accept="image/jpeg,image/png,image/webp,image/avif" className="hidden" onChange={(e) => e.target.files?.[0] && uploadProductImage(product.id, e.target.files[0])} />{savingProductId === product.id ? 'Subiendo imagen...' : 'Elegir imagen del dispositivo'}</label>
                      </div>
                      <div className="grid content-start gap-2 sm:grid-cols-3 lg:col-span-2 2xl:col-span-1 2xl:grid-cols-1">
                        <button type="button" onClick={() => updateProduct(product)} className="rounded-2xl bg-neutral-950 px-4 py-3 text-sm font-black text-white">{savingProductId === product.id ? 'Guardando...' : 'Guardar'}</button>
                        <button type="button" onClick={() => toggleProduct(product.id, product.available)} className={`rounded-2xl px-4 py-3 text-sm font-black ${product.available ? 'bg-emerald-100 text-emerald-700' : 'bg-cyan-50 text-neutral-700'}`}>{product.available ? 'Disponible' : 'No disponible'}</button>
                        <button type="button" onClick={() => setRecommendedProduct(product.id)} className={`rounded-2xl px-4 py-3 text-sm font-black ${product.recommended ? 'bg-[#037f86] text-white' : 'bg-[#049ca5] text-white'}`}>{product.recommended ? 'Recomendado' : 'Recomendar'}</button>
                      </div>
                    </div>
                  </div>
                ))}
                {!filteredAdminProducts.length && <p className="rounded-[28px] bg-white p-7 text-sm font-bold text-neutral-700 shadow-sm">No hay productos que coincidan con esos filtros.</p>}
              </div>
            </section>
          )}
        </section>
