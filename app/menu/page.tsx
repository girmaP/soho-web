  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 pb-28">
      ...
      <div className="mt-5 rounded-3xl border bg-white p-4 shadow-sm md:flex md:items-center md:justify-between">
        ...
        <a href="https://appcaylu.caylu.es/restaurantes/75" target="_blank" rel="noreferrer" className="mt-3 inline-flex rounded-2xl bg-[#049ca5] px-5 py-3 font-black text-white md:mt-0">Pedir a domicilio con Caylu</a>
      </div>

      {productsLoading && <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3" aria-busy="true" aria-label="Cargando productos">{Array.from({ length: 9 }).map((_, index) => <div key={index} className="h-72 animate-pulse rounded-[2rem] bg-white shadow-sm ring-1 ring-black/5" />)}</div>}
